...
namespace dxvk {
  ...
  {
    const size_t inputFormatId = size_t(inputFormat) <= formatInfoArray.size() ? size_t(inputFormat) : 0;

    const uint32_t inputDepth =
      ((formatInfoArray[inputFormatId].depth0 == formatInfoArray[inputFormatId].depth1)
    && (formatInfoArray[inputFormatId].depth0 == formatInfoArray[inputFormatId].depth2))
      ? formatInfoArray[inputFormatId].depth0
      : 0;
    ...
    if (inputDepth > 0 && formatInfoArray[size_t(targetFormat)].depth0 >= inputDepth)
      return targetFormat;
    else
      return inputFormat;
  }
